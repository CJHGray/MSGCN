...
def load_data(path="../data/acm/", dataset="acm"):
    ...
    # print(dense_a)
    # print(dense_a.shape)
    # print(dense_a[range(0,2)])

    # aaaa = np.array([[0,0,5],[1,1,1],[2,2,2]])
    # print(aaaa)
    # print(aaaa[range(1,2)])   得到的是[[1，1，1]]
    # print(aaaa[1])   得到的是[1，1，1]
    # print(aaaa[0][2])
    ...
    dddense_a = sparse_mx_to_torch_sparse_tensor(adj_a).to_dense()
    # print(dddense_a)
    # for co in range(0,1500):
    #     i = 0
    #     for index in range(0,1500):
    #         if dddense_a[co][index] != 0.0:
    #             dddense_a[co][i] = index
    #             i = i+1
    # print(dddense_a)
    ...
    idx_sample_adj = np.genfromtxt("{}{}.adj".format(path, dataset),
                                        dtype=np.dtype(str))
    # 读取../data/cora/cora.content
    sample_adj = sp.csr_matrix(idx_sample_adj[:, :], dtype=np.float32)
    sample_adj = torch.IntTensor(np.array(sample_adj.todense()))
    print(sample_adj)
    print(sample_adj.shape)

    sample = np.genfromtxt("{}{}.sample".format(path, dataset),
                                   dtype=np.dtype(str))
    # sample = torch.tensor(sample)
    sample = sp.csr_matrix(sample[:], dtype=np.float32)
    sample = torch.IntTensor(np.array(sample.todense()))
    for index in range(0, sample.shape[1]):
        sample[0][index] = sample[0][index] + 1
        if sample[0][index] > 10:
            sample[0][index] = 9
    ...
    return degree_a, degree_b, degree_c, adj_a, adj_b, adj_c, features, labels, idx_train, idx_val, idx_test, sample_adj, sample, idx_all
